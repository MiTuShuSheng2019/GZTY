...
- (void)statusBarOrientationChange{
     ...
     if (orientation == UIInterfaceOrientationLandscapeRight ||orientation == UIInterfaceOrientationLandscapeLeft) // home键靠左右
     {
         ...
         //采集--获取视频的宽高
         AVAsset *videoAsset = [AVAsset assetWithURL:[NSURL URLWithString:self.playerUrl]];
         ...
         if(naturalSize.height > naturalSize.width){
             videoW = naturalSize.height * CHPlayer_H/naturalSize.width;
         }else{
             videoW = naturalSize.width * CHPlayer_H/naturalSize.height;
         }
         
         CGFloat videoX = (CHPlayer_W - videoW)/2;
