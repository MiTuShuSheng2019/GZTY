...
- (instancetype)initWithTabTheme:(NBLScrollTabTheme *)tabTheme
{
    ...
    if (self) {
        ...
        _tabTheme.highlightColor = _tabTheme.highlightColor ? _tabTheme.highlightColor : [UIColor blueColor];
        _tabTheme.badgeViewColor = _tabTheme.badgeViewColor ? _tabTheme.badgeViewColor : [UIColor redColor];
        _tabTheme.titleViewBGColor = _tabTheme.titleViewBGColor ? _tabTheme.titleViewBGColor : [UIColor grayColor];
...
//        _tabTheme.titleViewBGColor = [UIColor whiteColor];
         _tabTheme.titleViewBGColor = RGB(238, 238, 238);
        _tabTheme.indicatorViewColor = [UIColor blueColor];
    }
...
- (void)tabView:(NBLScrollTabView *)tabView didSelectIndex:(NSUInteger)index
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(tabController:didSelectViewController:andIndex:)]) {
        [self.delegate tabController:self didSelectViewController:self.viewControllers[index] andIndex:index];
    }
}
