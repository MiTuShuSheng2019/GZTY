...
@interface TYGlobalSearchView ()<UIGestureRecognizerDelegate, UITextFieldDelegate>
...
@implementation TYGlobalSearchView
...
- (IBAction)ClickTimeSearch {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(ClickSearch:andStartTime:andEndTime:)]) {
        [_delegate ClickSearch:self.KeyWordTextField.text andStartTime:self.StartTimeTextField.text andEndTime:self.EndTimeTextField.text];
    }
    [self cancelView];
