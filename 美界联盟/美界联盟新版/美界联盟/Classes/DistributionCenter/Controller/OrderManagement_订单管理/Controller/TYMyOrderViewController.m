...
@interface TYMyOrderViewController ()<UIScrollViewDelegate, DEGlobalSearchViewDelegate>
...
@implementation TYMyOrderViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addNavigationBackBtn:@"back"];
    [self setNavigationBarTitle:@"我的订单" andTitleColor:[UIColor whiteColor] andImage:nil];
...
- (void)navigationRightBtnClick:(UIButton *)btn{
    DEGlobalSearchView *SearchView = [DEGlobalSearchView CreatDEGlobalSearchView];
...
-(void)ClickSearch:(NSString *)KeyWord andStartTime:(NSString *)startTime andEndTime:(NSString *)endTime andType:(NSString *)type{
    //通知传值
    NSNotification *not = [[NSNotification alloc] initWithName:@"searchMyOrderViewController" object:nil userInfo:@{@"keyWord":KeyWord,@"startTime":startTime,@"endTime":endTime,@"type":type}];
    [[NSNotificationCenter defaultCenter] postNotification:not];
}

-(void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"searchMyOrderViewController" object:nil];
}
