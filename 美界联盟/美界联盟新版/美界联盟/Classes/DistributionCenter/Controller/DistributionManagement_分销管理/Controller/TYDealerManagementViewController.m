//
//  TYDealerManagementViewController.m
//  美界联盟
//
...
@interface TYDealerManagementViewController ()<UIScrollViewDelegate, TYGlobalSearchViewDelegate>
...
@implementation TYDealerManagementViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addNavigationBackBtn:@"back"];
    [self setNavigationBarTitle:@"经销商管理" andTitleColor:[UIColor whiteColor] andImage:nil];
    [self setNavigationRightBtnImage:@"search"];
    
    self.view.backgroundColor = RGB(238, 238, 238);
    
    self.modelArray = [NSMutableArray arrayWithObjects:@"待审核", @"审核通过", nil];
    
    
    [self setUpTitleView];
    for (NSInteger i = 0; i < self.modelArray.count; i++) {
        DealerManagementViewController *vc = [[DealerManagementViewController alloc] init];
        vc.type = i + 1;
...
-(void)ClickSearch:(NSString *)KeyWord andStartTime:(NSString *)startTime andEndTime:(NSString *)endTime{
    //通知传值
    NSNotification *not = [[NSNotification alloc] initWithName:@"searchDealerManagementViewController" object:nil userInfo:@{@"keyWord":KeyWord,@"startTime":startTime,@"endTime":endTime}];
    [[NSNotificationCenter defaultCenter] postNotification:not];
}

-(void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"searchDealerManagementViewController" object:nil];
}
