...
@interface TYPersonResultsVC ()<UITableViewDelegate, UITableViewDataSource, TYAwaitingOneCellDelegate, TYGlobalSearchViewDelegate>
...
@implementation TYPersonResultsVC

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addNavigationBackBtn:@"back"];
    [self setNavigationBarTitle:@"个人业绩" andTitleColor:[UIColor whiteColor] andImage:nil];
    [self setNavigationRightBtnImage:@"search"];
    self.limit = 11;
    __weak typeof(&*self)weakSelf = self;
    self.tabView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        weakSelf.page = 1;
        [weakSelf requestPersonOrderCount];
    }];
...
-(void)requestPersonOrderCount{
    ...
    NSString *url = [NSString stringWithFormat:@"%@mapi/Order/personOrderCount",APP_REQUEST_URL];
...
-(void)requestMyorderList{
    ...
    params[@"e"] = @(-1);//订单状态 1未审核,2:审核通过;3:未通过;4:订单出库;5:订单入库;6:订单完成;7:订单作废
