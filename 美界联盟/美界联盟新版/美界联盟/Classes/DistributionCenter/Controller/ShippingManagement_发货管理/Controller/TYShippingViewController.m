...
@interface TYShippingViewController ()<UIScrollViewDelegate,TYGlobalSearchViewDelegate>
...
@implementation TYShippingViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self addNavigationBackBtn:@"back"];
    [self setNavigationBarTitle:@"发货" andTitleColor:[UIColor whiteColor] andImage:nil];
    [self setNavigationRightBtnImage:@"search"];
    
    self.view.backgroundColor = RGB(238, 238, 238);
    
    self.modelArray = [NSMutableArray arrayWithObjects:@"订单发货", @"流水发货", nil];
    
    [self setUpTitleView];
    for (NSInteger i = 0; i < self.modelArray.count; i++) {
        ShippingViewController *vc = [[ShippingViewController alloc] init];
        vc.type = i;
...
-(void)ClickSearch:(NSString *)KeyWord andStartTime:(NSString *)startTime andEndTime:(NSString *)endTime{
    //通知传值
    NSNotification *not = [[NSNotification alloc] initWithName:@"searchShippingViewController" object:nil userInfo:@{@"keyWord":KeyWord,@"startTime":startTime,@"endTime":endTime}];
    [[NSNotificationCenter defaultCenter] postNotification:not];
}

-(void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"searchShippingViewController" object:nil];
}
